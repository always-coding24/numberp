...
# =================================================================================
# Version: 1.4 (Smart Asset Reporting)
# Author: Israel & Gemini
# Description: A fully automatic, high-performance bot. This version
#              intelligently detects newly added ranges, automatically fetches
#              the full list of numbers within them, and sends this list as a
#              one-time report. It continues to monitor all ranges for new SMS.
# =================================================================================
...
class TelegramSender:
    ...
    def _worker(self):
        while not stop_event.is_set():
            try:
                chat_id, text, item_hash, is_sms = self.queue.get(timeout=1)
    ...
    def queue_message(self, chat_id, text, item_hash=None, is_sms=False):
        """Adds a message to the sending queue."""
        self.queue.put((chat_id, text, item_hash, is_sms))
...
def get_and_queue_number_list(session, termination_id, csrf_token, range_name):
    ...
    try:
        ...
        headers = {
            ...
            'Referer': MY_ACTIVE_SMS_PAGE_URL, 'X-CSRF-TOKEN': csrf_token,
        ...
        if numbers_data and isinstance(numbers_data, list):
            ...
            print(f"[!] No numbers found for new range '{range_name}' despite it being active.")
    except Exception as e:
...
def _process_and_queue_sms(phone_number, sender_cli, message_content, range_name):
    ...
    if sms_hash not in reported_sms_hashes_cache:
        ...
        telegram_sender.queue_message(DESTINATION_CHAT_ID, notification_text, sms_hash, is_sms=True)
...
def start_automatic_monitor(session):
    ...
    while not stop_event.is_set():
        try:
            ...
            current_active_ranges = {} # Use a dict to store name and ID
